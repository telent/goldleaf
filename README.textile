...
# The directory @template@ is where you put the files you want to install on the target system - if you have an existing system to take config from, copy interesting bits of its @/etc/ here. Do include: config files that you've changed from the defaults that Debian installed.  Don't include: anything that came out of a package and that you haven't changed since. If you leave this empty, you will get the Debian default @passwd@, @group@, and @shadow@ files (i.e. without any valid user accounts), fairly rudimentary @hosts@ and @fstab@ files, and an @apt/sources.list@ which has @PROXY@ written into it - fine if you are deploying on the same network as your proxy service, less so otherwise.
...
# boot the resulting image in kvm: either locally (there's a @make kvm@ target which may or may not fit your needs), or by copying it to your  "favourite kvm cloud provider":http://www.elastichosts.com (other cloud providers, doubtless, exist) and starting it there.
# wait for it to install everything.  It shouldn't need much interaction, but may need supervision as some packages insist on stopping and asking questions interactively despite my best efforts.
# reboot
